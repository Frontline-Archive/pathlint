...
function fileNameLint ( config, callback ) {
    ...
    var nodir         = ( typeof config.nodir != 'boolean' ? true : config.nodir ); // we exclude directories by default
    var removeFileExt = ( typeof config.removeFileExt != 'boolean' ? true : config.removeFileExt ); // we remove file extensions by default
